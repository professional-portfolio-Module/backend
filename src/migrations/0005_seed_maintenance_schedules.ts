export const name = '0005_seed_maintenance_schedules';

export const up = `
-- Alter the existing table to match the new seed data requirements
ALTER TABLE maintenance_schedules DROP CONSTRAINT IF EXISTS maintenance_schedules_frequency_check;
ALTER TABLE maintenance_schedules ADD CONSTRAINT maintenance_schedules_frequency_check CHECK (frequency IN ('daily','weekly','bi-monthly','monthly','quarterly','bi-annual','yearly','custom'));

DO \$\$
BEGIN
    IF EXISTS (SELECT 1 FROM information_schema.columns WHERE table_name='maintenance_schedules' AND column_name='interval') THEN
        ALTER TABLE maintenance_schedules RENAME COLUMN interval TO interval_days;
    END IF;
END \$\$;

ALTER TABLE maintenance_schedules ADD COLUMN IF NOT EXISTS week_labels TEXT;
ALTER TABLE maintenance_schedules ADD COLUMN IF NOT EXISTS occurrence_count INT NOT NULL DEFAULT 1;
ALTER TABLE maintenance_schedules ALTER COLUMN description DROP NOT NULL;
